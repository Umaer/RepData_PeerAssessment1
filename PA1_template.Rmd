...
activity_complete_sum_stepsbydate <- aggregate(steps ~ date, activity_complete, sum)
...
plot(activity_mean_stepsbyinterval$interval,activity_mean_stepsbyinterval$steps,type="l")
```
...
```{r}
library(lattice)
activity_full <- data.frame(day=weekdays(as.Date(activity_full$date)),daystatus="NA",activity_full)
activity_full$daystatus <- factor(ifelse(activity_full$day %in% c("Saturday","Sunday"),'weekend','weekday'))
```
...
activity_weekend_mean <- aggregate(steps ~ interval, activity_full_weekend, mean)
activity_weekday_mean <- aggregate(steps ~ interval, activity_full_weekday, mean)
...
xyplot(steps~interval  | daystatus, activity_allday_mean,type="l",layout=c(1,2))
```